...
public class UnitOwnerService:IUnitOwnerService
{
    ...
    { 
        _unitOwnerRepository.CreateUnitOwner(_mapper.Map<Entities.UnitOwner>(createUnitOwnerDto));
    }

    public async Task UpdateUnitOwner(int id, CreateUnitOwnerDto createUnitOwnerDto)
    {
        await _unitOwnerRepository.UpdateUnitOwner(id, _mapper.Map<Entities.UnitOwner>(createUnitOwnerDto));
    }
